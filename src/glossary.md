...
- `repr(Rust)` [enums] (without a primitive representation specified) with a single field-struct-like variant, a single unit-struct-like variant, or a single tuple-struct-like variant where the struct-like thing has no fields or where all of the fields are zero-sized (see [layout.repr.rust.enum-struct-like-zst]).
...
# /// An enum with a single field-struct-like variant with all fields being ZSTs.
enum E2 {
    V1 { f1: (), f2: [(); 10 ] },
}
...
# /// An enum with a single tuple-struct-like variant with all fields being ZSTs.
enum E5 {
    V1 ((), [(); 10 ]),
}
