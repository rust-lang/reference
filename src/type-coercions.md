...
1. `T` implements `Unsize<U>` or `CoerceUnsized<U>`.
2. `T` is present only in one of the fields of `S`.
3. ... and that field's type implements `CoerceUnsized<X>` where `X` is the type of the same field in `S<..., U,...>`.
