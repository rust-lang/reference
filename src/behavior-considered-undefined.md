...
* Breaking the pointer aliasing rules. The exact aliasing rules are not determined yet, but here is an outline of the general principles:
  ...
  * `Box<T, Global>` is treated similar to `&'static mut T` for the purpose of these rules.
  * `Box<T, A>` for other instances of `A` is treated similar to `&'static UnsafeCell<T>` for the purpose of these rules.

  These rules apply to *all* references and `Box`es, including those stored inside private fields (e.g., if your type has a private field of type `&mut T`, that reference must be unique in the sense described above for as long as values of your type are live).
