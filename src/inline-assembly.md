...
r[asm.argument.asm-syntax]
The exact assembly code syntax is target-specific and opaque to the compiler except for the way operands are substituted into the template string to form the code passed to the assembler.

r[asm.argument.syntax-form]
...
r[asm.operand-type.form]
Several types of operands are supported:

r[asm.operand-type.in]
* `in(<reg>) <expr>`
...
r[asm.operand-type.lateout]
* `lateout(<reg>) <expr>`
  - Identical to `out` except that the register allocator can reuse a register allocated to an `in`.
  - You should only write to the register after all inputs are read, otherwise you may clobber an input.

r[asm.operand-type.inout]
* `inout(<reg>) <expr>`
  - `<reg>` can refer to a register class or an explicit register.
    The allocated register name is substituted into the asm template string.
  - The allocated register will contain the value of `<expr>` at the start of the asm code.
...
r[asm.operand-type.inout-split]
...
r[asm.operand-type.inlateout]
* `inlateout(<reg>) <expr>` / `inlateout(<reg>) <in expr> => <out expr>`
  - Identical to `inout` except that the register allocator can reuse a register allocated to an `in` (this can happen if the compiler knows the `in` has the same initial value as the `inlateout`).
  - You should only write to the register after all inputs are read, otherwise you may clobber an input.

r[asm.operand-type.sym]
* `sym <path>`
...
r[asm.register-names.base-pointer]
...
r[asm.clobber_abi.constraint-class]
...
r[asm.options.checks-exclusive]
...
r[asm.directives.target-specific]

##### Dwarf Unwinding

r[asm.directives.target-specific.dwarf]
...
r[asm.directives.target-specific.x86]
...
r[asm.directives.target-specific.arm-32-bit]
